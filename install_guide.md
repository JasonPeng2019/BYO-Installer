...
byo uninstall --purge-data [--project <path>]
```
...
- refuses or warns if registered projects remain;
...
- removes verified product version directories;
- preserves project capsules and `.firm`;
- optionally preserves logs and update metadata for support;
- never recursively deletes an unresolved environment-variable path; and
- reports whether reinstall can recover existing projects.

### 22.3 Purge

`--purge-data` is separate and destructive. It must:

- list exact targets;
- reject broad roots;
- require explicit confirmation;
- distinguish global product data from project `.firm`;
- avoid following symlinks or reparse points;
- make clear that board profiles and evidence may be unrecoverable; and
- produce an uninstall receipt.
